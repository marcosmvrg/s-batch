...
public class DemonstrativoOrcamentarioWriterConfig {
	...
	public FlatFileItemWriter<GrupoLancamento> demonstrativoOrcamentarioWriter(
		...
		return new FlatFileItemWriterBuilder<GrupoLancamento>().name("demonstrativoOrcamentarioWriter")
				.resource(demonstrativoOrcamentario).lineAggregator(lineAggregator())
		...
		return new LineAggregator<GrupoLancamento>() {
			...
			public String aggregate(GrupoLancamento item) {
				String formatGrupoLancamento = String.format("[%d] %s - %s\n", item.getCodigoNaturezaDespesa(),
						item.getDescricaoNaturezaDespesa(), NumberFormat.getCurrencyInstance().format(item.getTotal()));

				StringBuilder str = new StringBuilder();
				str.append(formatGrupoLancamento);
