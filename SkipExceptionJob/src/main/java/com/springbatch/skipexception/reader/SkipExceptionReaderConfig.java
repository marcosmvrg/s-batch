...
public class SkipExceptionReaderConfig {
	...
	public ItemReader<Cliente> skipExceptionReader(@Qualifier("appDataSource") DataSource dataSource) {
		return new JdbcCursorItemReaderBuilder<Cliente>().name("skipExceptionReader").dataSource(dataSource)
				.sql("select * from cliente").rowMapper(rowMapper()).build();
	}
	...
	protected Cliente clienteRowMapper(ResultSet rs) throws SQLException {
		return new Cliente(rs.getString("nome"), rs.getString("sobrenome"), rs.getInt("idade"), rs.getString("email"));
	}
}
